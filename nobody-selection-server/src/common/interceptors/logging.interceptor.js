...
var LoggingInterceptor = function () {
    ...
    var LoggingInterceptor = _classThis = /** @class */ (function () {
        ...
        LoggingInterceptor_1.prototype.intercept = function (context, next) {
            ...
            var method = request.method, url = request.url, body = request.body, user = request.user;
            var userAgent = request.get('user-agent') || '';
            var ip = request.ip;
            var now = Date.now();
            this.logger.log("[".concat(method, "] ").concat(url, " - ").concat(userAgent, " ").concat(ip, " - User: ").concat((user === null || user === void 0 ? void 0 : user.id) || 'anonymous'));
            if (body && Object.keys(body).length > 0) {
                this.logger.debug("Request Body: ".concat(JSON.stringify(body)));
            }
